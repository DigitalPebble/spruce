...
Please note that the impact columns for the splits have a prefix `split_`, e.g `split_operational_energy_kwh` so that they are not counted twice: once with the resources and then again with the splits.
...
You can then query the CURs with the enriched splits. For instance to get the total of CO2eq emissions by EKS namespace: 


```SQL
create table spruce as select * from 'enriched_curs_with_splits/**/*.parquet';
