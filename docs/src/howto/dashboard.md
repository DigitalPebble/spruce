...
includes `dashboard.py`, a single-file [Streamlit](https://streamlit.io/)
...
| Trend | Energy, emissions, and water by billing period |
| Top emitters | Top product/service/operation combinations by operational emissions |
| Regions | Region, carbon intensity, PUE, water, energy, emissions, and gCO2 per dollar |
| Tags | Emissions, energy, and water split by a selected `resource_tags` key |
...
metrics or charts, edit the SQL query helpers in `dashboard.py`. They intentionally
match the query shapes in `report.py`, so changes can usually be copied between
the report and the dashboard.

`requirements-dashboard.txt` intentionally keeps versions unpinned, following
the lightweight style of `requirements-report.txt`. For reproducible deployments,
pin versions in your own environment or lock file.
...
`.streamlit/config.toml` and the `BRAND_COLOR` / `LOGO_PATH` constants in
`dashboard.py`. If you maintain a global `~/.streamlit/config.toml`, the
project-local file overrides it only while running from the repository root.
...
For a static report, use `report.py -o report.html` or `report.py -o report.pdf`.
