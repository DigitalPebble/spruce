...
The enrichment modules are listed and configured in a configuration file, one per cloud provider and report format. If no configuration is specified, SPRUCE uses the bundled default for the active provider (e.g. [`default-config-aws.json`](https://github.com/DigitalPebble/spruce/blob/main/src/main/resources/default-config-aws.json) for AWS, or [`default-config-azure-focus.json`](https://github.com/DigitalPebble/spruce/blob/main/src/main/resources/default-config-azure-focus.json) for an Azure [FOCUS 1.0](https://focus.finops.org/) report).
See [Configure the modules](howto/config_modules.md) for instructions on how to modify the enrichment modules.

An Azure FOCUS export carries the same values as the legacy cost details export under different
column names (e.g. `x_SkuMeterCategory` instead of `MeterCategory`, `ConsumedQuantity` instead of
`Quantity`). The Azure modules therefore work for both formats: when the report format is FOCUS,
they automatically rebind their input columns to the FOCUS names.
...
The FOCUS variant is provider-neutral: it reads the standard `RegionId` column.
...
**FOCUS Module**: `com.digitalpebble.spruce.modules.focus.RegionExtraction`
...
* **`water_consumption_stress_area_l`** – the total water consumption (`water_cooling_l` + `water_electricity_production_l`) attributed to regions under **high or extremely high water stress** (Aqueduct 4.0 baseline water stress category ≥ 3). This field is only populated when the electricity grid zone for the region has a water stress category of 3 (High) or 4 (Extremely High); it is absent otherwise. Water stress categories are derived from the [WRI Aqueduct 4.0](https://www.wri.org/data/aqueduct-global-maps-40-data) dataset.
...
**Azure Module**: `com.digitalpebble.spruce.modules.azure.FOCUSColumns`
