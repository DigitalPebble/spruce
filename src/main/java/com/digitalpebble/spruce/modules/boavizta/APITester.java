        ...
        if (args.length == 2) {
            ...
            try (java.io.BufferedWriter writer = java.nio.file.Files.newBufferedWriter(java.nio.file.Paths.get(args[1]))) {
                for (String instanceType : instanceTypes) {
                    ...
                    try {
                    double[] usageAndEmbodiedEnergy = client.getEnergyEstimates(Provider.AWS, instanceType.trim());
                    writer.write(instanceType + ", " + usageAndEmbodiedEnergy[0]);
                    writer.newLine();
                    } catch (Exception e) {
            ...
            double[] usageAndEmbodiedEnergy = client.getEnergyEstimates(Provider.AWS,args[0]);
