...
The `em-datacenters.json` + `averages_2024.csv` pair from
[Electricity Maps](https://www.electricitymaps.com) covers a similar
problem (carbon intensity per cloud region) but the two datasets are not
interchangeable. Joining them against the 157 regions produced by this
pipeline leaves 133 overlapping rows. Aggregate differences are small on
average (mean EM − Ember ≈ −7 gCO₂/kWh) but per-region dispersion is
large (MAE ≈ 55, RMSE ≈ 80 gCO₂/kWh). The reasons are structural:

**Nature of the value.** Electricity Maps' `averages_2024.csv` column is
`lifecycle`, which includes upstream supply-chain emissions (fuel
extraction, transport, plant construction). Ember's `CO2 intensity`
variable is power-sector direct/operational only. Lifecycle values are
typically higher than operational for the same grid; the uplift is
proportionally larger for renewable-heavy grids where operational
emissions are already low.

**Methodology and vintage.** Electricity Maps aggregates hourly data from
TSO/ISO feeds for calendar year 2024. Ember publishes annual values per
country and has 2025 available for most countries in the main release,
and several years back for the sub-national US and India releases. A
region whose grid has changed recently (new renewables, coal retirements)
can show a real difference of tens of gCO₂/kWh between the two sources
just from vintage.

**Location resolution.** The two systems disagree about what a "region"
means:

- Electricity Maps uses grid balancing authorities / market zones. In the
  US that means PJM, ERCOT, MISO, CAISO, SWPP, etc. — not states. In
  Canada it uses provinces (CA-AB, CA-ON, CA-QC). Outside NA it uses ISO
  3166-2 codes where available.
- This pipeline uses countries with sub-national breakdowns only where
  Ember publishes them (US states and Indian states) and resolves a
  cloud region's subdivision by reverse-geocoding its lat/lon.
- Electricity Maps ships ≈28 subdivision zones that at least one cloud
  datacenter falls into (11 US grid zones, 3 Canadian provinces, plus
  AU/BR/CL/IN/IT/JP/MY/NO/SE). Ember's sub-national resolution is
  currently US states and Indian states only; Canada and the rest fall
  back to country-level values in the Ember pipeline. This is the main
  source of large per-region gaps — e.g. `aws ca-west-1` (Calgary) is
  EM 402 (Alberta grid) vs Ember 191 (Canada national); `gcp us-east1`
  is EM 456 (CAR-SCEG) vs Ember 259 (South Carolina state).

**Coverage gaps.** 24 of the Ember pipeline's regions have no matching
entry in `em-datacenters.json` — mostly newer Azure regions
(`austriaeast`, `belgiumcentral`, `finlandcentral`, `chilecentral`,
`malaysiawest`, `saudiarabiasouthcentral`, `taiwannorth`, …), China, and
GovCloud / DoD / Sec regions.

**Rule of thumb.** For accurate per-region values in North America,
Electricity Maps' curated zone mapping is more trustworthy; for newer
regions or places it doesn't cover, Ember's country-level values (and
where available, state-level values) are the practical fallback.
